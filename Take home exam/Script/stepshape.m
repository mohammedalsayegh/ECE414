...
%
% [No,Do] = STEPSHAPE(...) returns the numerator polynomial vector No and
...
% poles are then scaled to provide the desired Ts. If Np is even, all poles
% are in complex conjugate pairs. If Np is odd, all poles except one are in
...
if ~isnumeric(n) || numel(n)>1 || fix(n)~=n || n<2 || n>15
   error('Np Must be a Scalar Integer Between 2 and 15.')
end
...
[err,p,tsa]=fun(alpha);                          %#ok<ASGLU> % gather specs at solution
